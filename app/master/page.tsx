...
export default async function MasterPage() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-gray-800">Vista Master — Todos los Agricultores</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white rounded-xl border border-gray-200 p-5 flex items-center gap-4">
          ...
          <div>
            <p className="text-xs text-gray-500">Total agricultores</p>
            <p className="text-2xl font-bold text-gray-800">{agricultores?.length ?? 0}</p>
          </div>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-5 flex items-center gap-4">
          <Wheat className="text-yellow-500" size={28} />
          <div>
            <p className="text-xs text-gray-500">Ha sembradas totales</p>
            <p className="text-2xl font-bold text-gray-800">{totalHaSembradas.toFixed(0)} ha</p>
          </div>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-5 flex items-center gap-4">
          <TrendingUp className="text-red-500" size={28} />
          <div>
            <p className="text-xs text-gray-500">Ha perdidas totales</p>
            <p className="text-2xl font-bold text-gray-800">{totalHaPerdidas.toFixed(0)} ha</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100">
          <h2 className="font-semibold text-gray-700">Agricultores del programa</h2>
        </div>
              ...
              {agricultores?.map(a => {
                ...
                return (
                  <tr key={a.AgricultorKey} className="hover:bg-gray-50">
                    <td className="px-4 py-3 font-medium text-gray-800">{a.nombre_agropecuaria}</td>
                    <td className="px-4 py-3 text-gray-500">{a.cedula ?? '—'}</td>
                    <td className="px-4 py-3 text-gray-500 text-xs">{a.correo_electronico ?? '—'}</td>
                    <td className="px-4 py-3 text-right text-gray-600">{lotes.length}</td>
                    <td className="px-4 py-3 text-right text-gray-600">{haSem.toFixed(1)}</td>
                    <td className="px-4 py-3 text-right text-red-500">{haPer.toFixed(1)}</td>
                    <td className="px-4 py-3 text-right font-medium text-green-700">{rend?.rendimiento?.toFixed(2) ?? '—'}</td>
                  </tr>
                )
              })}
            </tbody>
