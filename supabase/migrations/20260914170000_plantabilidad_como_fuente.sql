-- Exploracion completa pedida por el usuario (14-sep-2026): de las 53 tablas
-- del espejo saturno.*, plantabilidad es una tercera fuente ESTRUCTURADA de
-- fase (no texto libre) que hasta ahora no se usaba. Su columna
-- estado_fenologico es un id hash que referencia el catalogo
-- estado_fenologico (20 filas: V1..V14, R1..R6) -- ya sincronizado, nunca
-- resuelto.
--
-- Cobertura medida: plantabilidad tiene registros para 179 lotes del ciclo
-- activo, contra 110 de seguimiento -- casi el doble. De los lotes con
-- ambas fuentes, seguimiento suele ser mas reciente (80 vs 19 casos), pero
-- plantabilidad aporta cobertura donde seguimiento no tiene nada.
--
-- fase_final ahora compara TRES fuentes por fecha y usa la mas reciente:
-- seguimiento (visita formal), plantabilidad (visita de plantabilidad,
-- estructurada) y actividad (texto libre extraido de actividades_registro,
-- ver 20260914150000/160000). fase_fuente distingue las tres para que el
-- front sea honesto sobre el origen.

alter table public.lote_derivado
  add column if not exists fase_fecha_seguimiento date,
  add column if not exists fase_fecha_plantabilidad date;
...
begin
  ...
  -- Tercera fuente estructurada: plantabilidad + catalogo estado_fenologico.
  ...
  insert into public.lote_derivado (
    ...
    fase_fecha_seguimiento, fase_fecha_plantabilidad, updated_at
  )
  select
    uuid_generate_v5(v_ns, s.lote_id),
    -- Elegir la fase de la fuente con fecha mas reciente (empate: seguimiento > plantabilidad > actividad)
    (case
    ...
    null::numeric, -- placeholder, se recalcula abajo
    uv.fecha_visita,
    uv.tecnico,
    uv.observaciones,
    uv.acuerdos,
  ...
  -- avance_pct en un segundo paso: cosecha real > fase elegida > null.
...
-- Refrescar ya con la fuente nueva.
