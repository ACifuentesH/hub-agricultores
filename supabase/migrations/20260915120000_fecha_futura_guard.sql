-- Caso real encontrado (15-sep-2026): "El galpón" (lote L10-Y01, técnico
-- jesus.juarez@empresaspolar.com) llegó de grAIn con fecha_evaluacion
-- 2026-09-26 -- 11 días DESPUÉS de hoy, y también después de cargado_at
-- (2026-09-07). Es un typo del técnico en la planilla. Sin filtrar esto, esa
-- fecha falsa le ganaba a cualquier fuente real por tiempo indefinido (la
-- regla "gana la más reciente" no distingue una fecha futura de una
-- verdadera). Ya se corrigió en el origen (grain-estimaciones-sync clampea
-- fecha_evaluacion a cargado_at cuando es posterior), pero acá se agrega una
-- segunda barrera: cualquier fecha de cualquiera de las 4 fuentes que sea
-- posterior a hoy se descarta directo, sin importar de dónde venga.
...
begin
  ...
  with ultima_visita as (
    ...
    where s.ciclo_cosecha_id = v_ciclo_id
      and (public.saturno_fecha(nullif(s.fecha_seguimiento, '')) is null
           or public.saturno_fecha(nullif(s.fecha_seguimiento, '')) <= current_date)
    order by
      s.lote_id,
  ...
  ultima_actividad as (
    ...
    where a.ciclo_id = v_ciclo_id
      and (public.saturno_fecha(nullif(a.fecha_inicio, '')) is null
           or public.saturno_fecha(nullif(a.fecha_inicio, '')) <= current_date)
    order by
      a.lote_id,
  ...
  ultima_plantabilidad as (
    ...
    where p.ciclo_cosecha_id = v_ciclo_id
      and (public.saturno_fecha(nullif(p.fecha_seguimiento, '')) is null
           or public.saturno_fecha(nullif(p.fecha_seguimiento, '')) <= current_date)
    order by
      p.lote_id,
  ...
  grain_fase as (
    ...
    where g.ciclo = v_ciclo_id
      ...
      and (g.fecha_evaluacion is null or g.fecha_evaluacion <= current_date)
  )
